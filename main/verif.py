        ...
        async def button_callback(inter: discord.Interaction):
            ...
            if data[0] is not None:
                ...
                if faceit_level:
                    ...
                    faceit_role = get(
                        inter.guild.roles, id=faceit_role_id)
                    roles.append(
                        f"{faceit_role.mention} {faceit_icon}")
                ...
                if valid_role:
                    roles.append(
                        f"{valid_role.mention} <:valid:1159335185830125588>")
                ...
                if steamid:
                    ...
                    roles.append(
                        f"{timerole.mention} <:time:1160165904080977920>")
                    roles_to_add.append(timerole)

                await inter.user.add_roles(*roles_to_add)

                embed = discord.Embed(title="Вы прошли верификацию!", description=f"Вы получили роли: {', '.join(roles)}", color=discord.Colour.blurple(
                ), timestamp=datetime.datetime.now())
                embed.set_thumbnail(
                    url=inter.user.avatar.url if inter.user.avatar else inter.user.default_avatar)
